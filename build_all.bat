...
REM
REM Order:
REM   1. tool       --> revival_tool.exe         (32-bit console)
REM   2. patcher    --> revival_patcher.dll      (32-bit DLL, MinHook bundled)
REM   3. wrapper    --> revival_wrapper.exe      (32-bit GUI/console, IFEO target)
REM
...
if "%OK%"=="1" (
    ...
    echo.
    echo Artifacts:
    if exist "%ROOT%tool\revival_tool.exe"          echo   tool\revival_tool.exe
    if exist "%ROOT%patcher\revival_patcher.dll"    echo   patcher\revival_patcher.dll
    if exist "%ROOT%wrapper\revival_wrapper.exe"    echo   wrapper\revival_wrapper.exe
    echo.
    echo Quick test:
    echo   tool\revival_tool.exe ping
    exit /b 0
) else (
